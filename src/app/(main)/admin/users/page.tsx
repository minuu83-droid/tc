...
type ChangeState = 'idle' | 'saving' | 'done' | 'error';
...
export default function AdminUsersPage() {
  ...
  const [stateMap, setStateMap] = useState<Record<string, ChangeState>>({});
  ...
  /* ── 역할 변경 저장 ── */
  const saveRole = async (targetId: string) => {
    if (!profile) return;
    const newRole = roleMap[targetId];
    const original = users.find(u => u.id === targetId)?.role;
    ...
    if (rpcErr || (data as { error?: string })?.error) {
      ...
      setError(rpcErr?.message ?? (data as { error?: string }).error ?? '오류 발생');
      // 원래 값으로 되돌리기
      setRoleMap(m => ({ ...m, [targetId]: original! }));
  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">전체 사용자의 역할을 조회하고 변경합니다.</p>
      </div>
              ...
              return (
                <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 font-medium text-gray-900">
                    {user.name}
                    {isMe && (
                      <span className="ml-1.5 text-xs text-blue-500 font-normal">(나)</span>
                    )}
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <select
                      ...
                      disabled={s === 'saving'}
                      className={[
                        'text-xs border rounded-lg px-2 py-1.5 pr-6 focus:outline-none focus:ring-2 focus:ring-blue-300 transition-colors',
                        changed ? 'border-blue-400 bg-blue-50' : 'border-gray-200 bg-white',
      ...
      <p className="mt-3 text-xs text-gray-400">
        총 {users.length}명 · 역할 변경 후 <strong>저장</strong> 버튼을 클릭해야 DB에 반영됩니다.
      </p>
    </div>
  );
}
